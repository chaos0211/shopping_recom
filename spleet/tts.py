...
# 加载背景音乐并降低音量
background_music = AudioSegment.from_file("backmu.mp3") - 5
...
async def main():
    ...
    for part in parts:
        if part["type"] == "text":
            ...
            # 循环背景音乐长度覆盖 segment
            loop_count = (len(segment) // len(background_music)) + 1
            bgm_loop = background_music * loop_count
            segment_with_bgm = segment.overlay(bgm_loop[:len(segment)])
            audio += segment_with_bgm
            os.remove(temp_file)
            idx += 1
    ...
    audio.export("output_with_backup.mp3", format="mp3")
    print("✅ 生成成功：output_with_backup.mp3")
